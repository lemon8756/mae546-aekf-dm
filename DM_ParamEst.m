...
% Created on May 5, 2015, 16:30 by Aaron James Lemmer
...
%     addKolm = newKolm*0.5*exp(-(iteration-1)/10);
    ...
    phsest = phsest + DMbasmodvec*comvec;
    ...
    covest = covest + actvar*(DMbasmodvec*DMbasmodvec');
    
    % Compute the filter gains
    gainK = covest/(covest + R);
    
    % Update the state estimate with a measurement
    phsest = phsest + gainK*(phsmeas - phsest);
    phsestmat = reshape(phsest,[nxy,nxy]);
